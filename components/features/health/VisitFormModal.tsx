...
// import RichTextInput from './shared/RichTextInput'; // Verify path
import RichTextInput from '@/components/ui/RichTextInput';
import PlacesAutocomplete from '@/components/ui/PlacesAutocomplete';
import FormModal, { FormState } from '@/components/ui/FormModal';
import BottomSheetSelect from '@/components/ui/BottomSheetSelect';
import { useLocale } from '@/hooks/useLocale';
import { useCreateVisit, useUpdateVisit, useCreateActivityLog } from '@/hooks/domain/useHealthMutationsV2';
import { MedicalVisit } from '@/types/v2/schema';

interface VisitFormModalProps {
    visible: boolean;
    onClose: () => void;
    petId?: string;
    existingVisit?: MedicalVisit | null; // V2 Type
    onSuccess?: () => void;
}
...
export default function VisitFormModal({ visible, onClose, petId: initialPetId, onSuccess }: VisitFormModalProps) {
    ...
    // const { mutateAsync: updateVisit } = useUpdateVisit(); // Add if editing needed
    ...
    const handleSubmit = async (data: VisitFormData) => {
        ...
        const visitData: any = { // Cast relevant to Omit<MedicalVisit...>
            pet_id: selectedPetId,
            visit_type: data.service_category, // V2 uses visit_type? Check Schema.
            // V2 Schema medical_visits: id, pet_id, visit_type, visit_date, reason, provider_id, cost, currency, notes.
            // NO provider_type, urgency, symptoms, diagnosis in V2 Schema directly?
            // "diagnosis" might be notes?
            // "symptoms" might be notes?
            // "provider_type"?
            // We might have to stuff these into NOTES or METADATA?
            // Or V2 Schema is simpler.
            // "visit_type" -> service_category.
            visit_date: data.date,
            reason: data.reason || data.service_category,
            // provider_id ... again, logic for text provider. Omit for now or implement Provider creation.
            cost: data.cost ? parseFloat(data.cost) : null,
            currency: data.currency,
            notes: finalNotes || null,
            // status: 'scheduled' | 'completed' | 'cancelled'; // Default completed?
            status: 'completed'
        };
        ...
        try {
            ...
            await logActivity({
                ...
                activity_type: 'visit',
                title: 'Visit Added',
                description: `${data.service_category} at ${data.business_name}`,
                metadata: {
...
const styles = StyleSheet.create({
    formContent: { gap: 16 },
    section: { gap: 8 },
    sectionLabel: { fontSize: 11, fontWeight: '600', letterSpacing: 0.5, marginBottom: 4, textTransform: 'uppercase' },
    providerRow: { flexDirection: 'row', gap: 12 },
    providerItem: { alignItems: 'center', minWidth: 64 },
    providerIcon: { width: 48, height: 48, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginBottom: 6, borderWidth: 1 },
    providerLabel: { fontSize: 11, fontWeight: '500', textAlign: 'center' },
    ...
    urgencyButton: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 10, borderWidth: 1 },
    urgencyText: { fontSize: 12, fontWeight: '600', textTransform: 'uppercase', fontFamily: 'Plus Jakarta Sans' },
    chipRow: { flexDirection: 'row', gap: 8, flexWrap: 'wrap' },
    categoryChip: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, borderWidth: 1 },
    categoryChipText: { fontSize: 13, fontWeight: '500', fontFamily: 'Plus Jakarta Sans' },
});
