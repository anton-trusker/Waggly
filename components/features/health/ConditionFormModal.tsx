...
import { useAppTheme } from '@/hooks/useAppTheme'; // Or designSystem, maintaining legacy choice
...
import { Condition } from '@/types/v2/schema';
import { useCreateCondition, useUpdateCondition, useCreateActivityLog } from '@/hooks/domain/useHealthMutationsV2';
...
export default function ConditionFormModal({ visible, onClose, petId: initialPetId, existingCondition, onSuccess }: ConditionFormModalProps) {
    ...
    const initialData = useMemo(() => {
        if (existingCondition) {
            return {
                ...
                status: existingCondition.status as any,
    ...
    const handleSubmit = async (data: ConditionFormData) => {
        ...
        const payload: any = { // Cast Omit<Condition...>
        ...
        try {
            if (existingCondition) {
                await updateCondition({ id: existingCondition.id, updates: payload });
                await logActivity({
                    pet_id: selectedPetId,
...
const styles = StyleSheet.create({
    ...
    input: { borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, fontSize: 16, borderWidth: 1 },
