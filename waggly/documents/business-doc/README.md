# Waggli - Платформа помощи питомцам

Waggli - это комплексная веб-платформа, которая соединяет владельцев животных с людьми, готовыми помочь. Платформа позволяет публиковать объявления о различных видах помощи: финансовой, донорстве крови и физической помощи для животных.
...
1. Клонировать репозиторий:
   ```
   git clone https://github.com/your-username/waggli.git
   cd waggli
   ```
...
3. Создать файл с переменными окружения `.env`:
   ```
   DATABASE_URL=postgresql://username:password@localhost:5432/waggli
