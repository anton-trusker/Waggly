...
export function usePetSharing(petId?: string) {
    ...
    const validateToken = async (token: string): Promise<{ data: SharedPetData | null, error: any }> => {
        try {
            ...
            const { data: petData, error: petError } = await supabase
                .from('pets')
                .select('id, name, species, breed, gender, date_of_birth, photo_url, microchip_number')
    ...
    const getShareUrl = (token: string) => {
        let baseUrl = 'https://app.mypawzly.app'; // Production default
        ...
        if (typeof window !== 'undefined') {
            ...
            } else if (hostname.includes('pawzly.eu')) {
                baseUrl = 'https://app.pawzly.eu'; // Staging
            }
